...
def setup_hf_model(model_name, cache_dir='/disk1/', max_new_tokens=7000, dry_run_on_history=False):
    """
    ...
    if not dry_run_on_history:
        ...
        with torch.no_grad():
            if "llama" in model_name.lower():
                print('loading llama')
                model = AutoModelForCausalLM.from_pretrained(model_name, config=config, cache_dir=os.path.join(cache_dir, model_name), device_map='auto', torch_dtype="bfloat16")
            else:
                model = AutoModelForCausalLM.from_pretrained(model_name, config=config, cache_dir=os.path.join(cache_dir, model_name), device_map='auto', torch_dtype="auto")
        model.eval()
...
def randomize_agents_order(agents, p1, rounds, single_agent=False):
    round_assign = []
    names = [name for name in agents.keys()]
    if single_agent:
        single_agent_name = next(name for name, details in agents.items() if details['role'] == 'p1')
        round_assign = [single_agent_name] * rounds
    else:
        last_agent = p1
