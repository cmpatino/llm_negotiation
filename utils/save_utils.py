...
def process_agents(agents):
    ...
    for agent_name, values in agents.items():
        agent_dict = {}
        agent_dict['agent'] = agent_name
        agent_dict['role'] = values['role']
        agent_dict['model'] = values['model']
        agent_dict['incentive'] = values['incentive']
        agent_dict['initial_prompt'] = values['instance'].initial_prompt
...
def save_conversation(history, agent_name, full_answer, prompt, agents={}, round_assign=[], initial=False):

    if initial:
        history['content']['agents'] = process_agents(agents)
